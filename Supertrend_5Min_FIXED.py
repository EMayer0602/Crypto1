...
def _maybe_append_synthetic_bar(df, symbol, timeframe):
	...
	IMPORTANT: The synthetic bar uses current_start as index (beginning of bar period)
	so that signals are calculated DURING the bar, not AFTER it closes.

	OHLC calculation from 1m bars since last full hour:
	- Open = Open of first 1m bar after last full hour
	...
	# current_start = beginning of current incomplete bar (last full hour/period)
	current_start = now.floor(f"{tf_minutes}min")
	current_end = current_start + bucket

	# Check if we already have this bar - remove it to recreate with latest data
	if not df.empty:
		last_idx = df.index.max()
		# If last bar is current_start, remove it (we'll recreate with updated data)
		if last_idx >= current_start:
			df = df[df.index < current_start]

	# Calculate how many 1m bars we need since current_start
	minutes_elapsed = max(2, int(np.ceil((now - current_start).total_seconds() / 60.0)) + 2)

	# Try to fetch 1-minute bars first (most accurate)
	try:
		minute_df = _fetch_direct_ohlcv(symbol, "1m", limit=minutes_elapsed + 5)
		# Include bars from current_start (>=) not just after (>)
		slice_df = minute_df[(minute_df.index >= current_start) & (minute_df.index <= now)]

		if not slice_df.empty:
			synthetic = pd.DataFrame({
				"open": float(slice_df["open"].iloc[0]),      # First bar's open
			...
			}, index=[current_start])  # Index = START of bar, not end!

			combined = pd.concat([df, synthetic])
			combined = combined[~combined.index.duplicated(keep="last")]
			combined = combined.sort_index()

			print(f"[Synthetic] Live bar for {symbol} {timeframe}: O={slice_df['open'].iloc[0]:.2f} H={slice_df['high'].max():.2f} L={slice_df['low'].min():.2f} C={slice_df['close'].iloc[-1]:.2f} ({len(slice_df)} 1m bars, {current_start.strftime('%H:%M')}-now)")
			return combined
	except Exception as exc:
	...
	try:
		...
		if ticker and 'last' in ticker and ticker['last']:
			...
			# Use previous close as open price (best estimate)
			if not df.empty:
			...
			synthetic = pd.DataFrame({
				"open": synthetic_open,
				"high": max(synthetic_open, last_price),  # At least cover range
				"low": min(synthetic_open, last_price),   # At least cover range
				"close": last_price,
				"volume": 0.0,
			}, index=[current_start])  # Index = START of bar, not end!
			...
			print(f"[Synthetic] Live bar for {symbol} {timeframe} (ticker): O={synthetic_open:.2f} C={last_price:.2f} ({current_start.strftime('%H:%M')}-now)")
			return combined
	except Exception as exc:
