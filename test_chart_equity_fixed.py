...
def test_equity_chart(symbol='XRP-EUR'):
    ...
    if 'equity_curve' in result:
        ...
        print(f"📊 Equity in result: {len(equity)} values, €{equity[0]:.0f} -> €{equity[-1]:.0f}")
    
    if 'buyhold_curve' in result:
        buyhold = result['buyhold_curve']
        print(f"📈 BuyHold in result: {len(buyhold)} values, €{buyhold[0]:.0f} -> €{buyhold[-1]:.0f}")
